...
get_data(api=api, dataset_name="se-7", date="2015", return_object=FALSE)
```
...
```{r}
library(svgtools)
svg <- get_svg(api=api, dataset_name="se-7", date="2015")
display_svg(svg)
```
